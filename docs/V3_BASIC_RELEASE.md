...
本次测试不调用真实模型。真实 API 生成、搜索质量、费用和 5–10 个 Ideas 的评估仍待验证。代码已推送到 `codex/v3-basic-release` 并建立草稿 PR #1，尚未部署到线上。

GitHub Linux CI 初次运行发现 `pytest` 可执行入口没有将后端目录加入模块搜索路径，导致所有测试无法导入 `app`。CI 已改用与本地验证一致的 `python -m pytest -q`；前端 CI 构建已通过。
