...
from .orchestration.runner import GraphRunner
...
def create_app(db_path=None, provider=None, settings=None):
    ...
    if config['workflow_engine'] not in {'v2', 'langgraph'}:
        raise ValueError('WORKFLOW_ENGINE must be v2 or langgraph.')
    ...
    runner = GraphRunner(db, generator)
    ...
    async def lifespan(app):
        ...
        db.init()
        # Also drain existing V3 approvals after a feature-flag rollback. Never
        # silently disable recovery for persisted LangGraph work.
        runner.setup()
        runner.start()
        try:
            yield
    ...
    app = FastAPI(title='Idea Atelier API', version='3.0.0', lifespan=lifespan)
    ...
    def generate(project_id: str, stage: Stage, body: GenerateRequest, background: BackgroundTasks,
        ...
        try:
            run_id, project, context, previous = db.prepare_run(project_id, user['id'], stage, STAGES,
                body.feedback, config['daily_limit'], graph=config['workflow_engine'] == 'langgraph', model=generator.model)
        except DatabaseError as exc:
            fail(exc)
        if config['workflow_engine'] == 'langgraph':
            background.add_task(runner.execute, run_id)
        else:
    ...
    def accept(project_id: str, stage: Stage, body: AcceptRequest, background: BackgroundTasks, user=Depends(current_user)):
        try:
            result = db.accept(project_id, user['id'], stage, STAGES, body.artifact_id, body.decision, body.revised_idea)
            for run in result['runs']:
                if run.get('engine') == 'langgraph' and run.get('workflow_status') == 'queued':
                    background.add_task(runner.execute, run['id'])
            return result
        except DatabaseError as exc:
            fail(exc)

    @app.post('/api/projects/{project_id}/runs/{run_id}/recover', status_code=202)
    def recover(project_id: str, run_id: str, background: BackgroundTasks, user=Depends(current_user)):
        try:
            db.recover_graph_approval(project_id, user['id'], run_id)
        except DatabaseError as exc:
            fail(exc)
        background.add_task(runner.execute, run_id)
