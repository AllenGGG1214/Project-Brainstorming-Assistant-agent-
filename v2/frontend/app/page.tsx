...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (!disposed) {
          setProject(data);
          const activeRun = data.runs.some(r => r.status === 'running' || ['queued', 'executing'].includes(r.workflow_status || ''));
          if (activeRun) timer = setTimeout(poll, 1200);
          else await loadList();
        }
  ...
  async function recoverWorkflow() {
    ...
    setBusy(true); setError('');
    try {
      await api(`/projects/${project.id}/runs/${latestRun.id}/recover`, {});
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="workspace-note"><span className="status-dot" /><span>{auth?.required ? 'Private workspace' : 'Local workspace'}<small>Projects are saved automatically</small></span><span className="version-chip">V3</span></div><button className="settings-button" onClick={() => setSettings(true)}><Settings2 size={16} />Connection & settings<ArrowUpRight size={14} /></button></div>
    </aside>
    ...
    <main className="main">
      ...
      {!selected ? <div className="overview">
        ...
        <footer className="overview-footer"><span>Evidence before assumptions. Clarity before code.</span><span>IDEA ATELIER / V3</span></footer>
        ...
        <div className="work-grid"><section className="artifact-panel"><div className="artifact-title"><div><StageIcon size={19} /><h2>{stageLabels[index]}</h2>{artifact && <span className="tag">v{artifact.version} · {artifact.approved_at ? 'Accepted' : 'Draft'}{!artifact.valid ? ' · History' : ''}</span>}</div><button className={'icon-button ' + (history ? 'chosen' : '')} aria-label="Version history" onClick={() => setHistory(!history)}><History size={18} /></button></div>
          ...
          {running?.stage === stage && <div className="generating" role="status"><LoaderCircle className="spin" size={18} /><div><strong>Generating {stageLabels[index]}…</strong><small>You can switch projects or refresh. Your progress is saved. Interrupted work will be checked when the service resumes.</small></div></div>}
          {latestRun?.workflow_status === 'waiting_approval' && current && !current.approved_at && <div className="alert" role="status">Generation is saved and waiting for your review. Accept this version or generate a revision.</div>}
          {latestRun?.workflow_status === 'needs_attention' && latestRun.status === 'succeeded' && <div className="alert error" role="alert">Your artifact is saved, but workflow synchronization needs attention.  <button className="secondary" disabled={busy} onClick={recoverWorkflow}>Retry synchronization</button></div>}
        ...
        </section><aside className="action-rail"><section className="rail-card"><span className="eyebrow">NEXT STEP</span><h3>{stopped ? 'Exploration paused' : current?.approved_at ? 'Current artifact accepted' : current ? 'Shape the artifact around your idea' : 'Ready for the next step'}</h3><p>{stopped ? 'Review the saved artifacts and history, or export the design pack.' : current?.approved_at ? 'Continue to the next stage, or generate a revision with feedback.' : 'You accept each stage before moving forward.'}</p>
